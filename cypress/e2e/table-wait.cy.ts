...
rowTestRuns.forEach(run => {
	describe(`TableWait ${run.rowKind}`, () => {
		it("default options should not throw error with timeout", () => {
			...
			TableWait.waitForRows(cy.get("table"), run.rowSelector, run.cellSelector, run.rowKind, { timeout: 2000 });
		});
		...
		it("default options should throw error without timeout", () => {
			...
			cy.once("fail", error => {
				expect(error.message).to.include(`No ${run.rowKind} cells with content found`);
				return false;
			});

			TableWait.waitForRows(cy.get("table"), run.rowSelector, run.cellSelector, run.rowKind);
		});
		...
		it("should throw error when amount of rows is not met", () => {
			...
			cy.once("fail", error => {
				expect(error.message).to.include(`Expected 5 ${run.rowKind} rows, but found ${run.expectedRowAmount}`);
				return false;
			});
		...
		it("should throw error when amount of cells is not met", () => {
			...
			cy.once("fail", error => {
				expect(error.message).to.include(`Expected amount of 10 ${run.rowKind} cells for row not found`);
				return false;
			});
		...
		it("should throw error when amount of cells with content is not met", () => {
			...
			cy.once("fail", error => {
				expect(error.message).to.include(`Expected amount of 10 ${run.rowKind} cells with content for row not found`);
				return false;
			});
